...
var schedules: IScheduleDefinition[] = [{ // Facility Holidays
    ...
    child_schedules: [{ sys_id: "c7475f021ba7c9101497a820f54bcb04", type: 'include' }]
}, { // Core Hours
    ...
    child_schedules: [{ sys_id: "46981f421ba7c9101497a820f54bcbff", type: 'include' }]
}, { // Personnel Hours
    ...
    child_schedules: [{ sys_id: "46981f421ba7c9101497a820f54bcbff", type: 'include' }]
}, { // Facility Hours, All Weekdays (includes holidays)
    sys_id: "21e4db8e1b67c9101497a820f54bcb7a",
    is_holiday: false,
    entries: [{
        name: "0600-1800 Weekdays",
        start_date_time: "20180222T060000",
    ...
    child_schedules: []
}, { // Personnel Hours, All Weekdays (includes holidays)
    sys_id: "83735f4e1b67c9101497a820f54bcb55",
    is_holiday: false,
    entries: [{
        name: "0800-1200 Weekdays",
        start_date_time: "20080707T080000",
        end_date_time: "20080707T120000",
        all_day: false,
        repeat_type: "weekdays",
        notes: 'Represents before-lunch hours. Actual hours may vary for each person.'
    }, {
    ...
    child_schedules: []
}, { // Facility Hours, Every Day (includes weekends and holidays)
    sys_id: "a255d7ce1b67c9101497a820f54bcbee",
    is_holiday: false,
    entries: [{
        name: "0800-1800 Daily",
        start_date_time: "20180222T060000",
        end_date_time: "20180222T180000",
        all_day: false,
        repeat_type: "daily"
...
for (var s = 0; s < schedules.length; s++) {
    ...
    if (cmn_schedule.is_holiday) {
        ...
        for (var i = 0; i < cmn_schedule.entries.length; i++) {
            ...
            if (!gs.nil(entry.month)) gr.setValue('month', entry.month);
            if (!gs.nil(entry.notes)) gr.setValue('notes', entry.notes);
        ...
        if (rowCount > 0)
            gs.warn("cmn_schedule_span[schedule='" + cmn_schedule.sys_id + "'] - Expected row count: " + cmn_schedule.entries.length + "; Actual: " + rowCount);
        else
    ...
    for (var c = 0; c < cmn_schedule.child_schedules.length; c++) {
        ...
        if (!gr.next()) {
            ...
            gr.newRecord();
            gr.setValue('schedule', cmn_schedule.sys_id);
            gr.setValue('child_schedule', cmn_schedule.sys_id);

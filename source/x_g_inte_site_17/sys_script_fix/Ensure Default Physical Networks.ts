namespace ensure_default_physical_networks {
	interface IRecordDefinition {
		sys_id: string;
		name: string;
		description: string;
	}
	var recordDefinitions: IRecordDefinition[] = [
	...
	for (var i = 0; i < recordDefinitions.length; i++) {
		var gr: GlideRecord = new GlideRecord('x_g_inte_site_17_site_17_network_circuit');

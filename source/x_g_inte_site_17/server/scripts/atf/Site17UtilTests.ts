...
namespace site17Util_IteratorFromArrayTest {
    ...
    interface ITestParamGroup {
        values: (string | INextWithArg)[];
    ...
    (function(outputs: sn_atf.ITestStepOutputs, steps: sn_atf.ITestStepsFunc, stepResult: sn_atf.ITestStepResult, assertEqual: sn_atf.IAssertEqualFunc): boolean {
        ...
        function createIterator(this: ITargetInfo, callInfo: ICallInfo, onThrow?: GlideDuration | null, prefix?: string): boolean {
            callInfo.throwCalled = false;
            callInfo.thrown = undefined;
            this.onThrow = onThrow;
            this.values = this.iterations.map(function(item: string | INextWithArg): string { return (typeof item === 'string') ? item : item.value });
            if (typeof onThrow === 'undefined') {
                if (typeof this.finalReturnValue === 'undefined') {
                    if (typeof this.supportsReturn === 'undefined') {
                        ...
                        try { this.iterator = x_g_inte_site_17.Site17Util.iteratorFromArray<string, GlideDuration, number>(this.values); }
                        catch (e) {
                        ...
                        try {this. iterator = x_g_inte_site_17.Site17Util.iteratorFromArray<string, GlideDuration, number>(this.values, this.supportsReturn); }
                        catch (e) {
                            atfHelper.setFailed('Unexpected exception while invoking ' + this.pseudoCode, e);
                            return false;
                        }
                    ...
                    try { this.iterator = x_g_inte_site_17.Site17Util.iteratorFromArray<string, GlideDuration, number>(this.values, this.supportsReturn, this.finalReturnValue); }
                    catch (e) {
                        atfHelper.setFailed('Unexpected exception while invoking ' + this.pseudoCode, e);
                        return false;
                    }
                ...
                var onThrowFunc: { (e?: any): GlideDuration | undefined; } | undefined;
                if (onThrow === null) {
                    onThrowFunc = function(e?: any): GlideDuration | undefined {
                ...
                try { this.iterator = x_g_inte_site_17.Site17Util.iteratorFromArray<string, GlideDuration, number>(this.values, this.supportsReturn, this.finalReturnValue, onThrowFunc); }
                catch (e) {
                    atfHelper.setFailed('Unexpected exception while invoking ' + this.pseudoCode, e);
                    return false;
                }
        ...
        function testIterations(this: ITargetInfo, count: number): boolean {
        ...
        function testIteratorFromArray(iterations: (string | INextWithArg)[], supportsReturn?: boolean, finalReturnValue?: GlideDuration, onThrow?: GlideDuration | null): boolean {
            var callInfo: ICallInfo = { throwCalled: false };
            var targetInfo = <ITargetInfo>{
                iterations: iterations,
                supportsReturn: supportsReturn,
                finalReturnValue: finalReturnValue
            };
            if (!(createIterator.call(targetInfo, callInfo, onThrow) && testIterations.call(targetInfo, iterations.length))) return false;

            var iterationPseudoCode = targetInfo.pseudoCode + '[' + targetInfo.values.length + '].next()';
            var ir: IteratorResult<string, GlideDuration>;
            try { ir = targetInfo.iterator.next(); }
            catch (e) {
                atfHelper.setFailed('Unexpected exception while invoking ' + iterationPseudoCode, e);
                return false;
            }
            ...
            if (supportsReturn === true) {
                ...
                if (iterations.length > 1) {
                    if (!(createIterator.call(targetInfo, callInfo, onThrow, "Return") && testIterations.call(targetInfo, iterations.length - 1))) return false;
                    iterationPseudoCode = targetInfo.pseudoCode + '.return()';
                    try { ir = (<{ (value?: GlideDuration): IteratorResult<string, GlideDuration>; }>targetInfo.iterator.return)(); }
                    catch (e) {
                        atfHelper.setFailed('Unexpected exception while invoking ' + iterationPseudoCode, e);
                        return false;
                    }
                    ...
                    try { ir = targetInfo.iterator.next(-1); }
                    catch (e) {
                        atfHelper.setFailed('Unexpected exception while invoking ' + iterationPseudoCode, e);
                        return false;
                    }
                    ...
                    if (!(createIterator.call(targetInfo, callInfo, onThrow, "Return") && testIterations.call(targetInfo, iterations.length))) return false;
                    iterationPseudoCode = targetInfo.pseudoCode + ".return(new GlideDuration('0 0:0:0'))";
                    try { ir = (<{ (value?: GlideDuration): IteratorResult<string, GlideDuration>; }>targetInfo.iterator.return)(duration); }
                    catch (e) {
                        atfHelper.setFailed('Unexpected exception while invoking ' + iterationPseudoCode, e);
                        return false;
                    }
                ...
                if (!createIterator.call(targetInfo, callInfo, onThrow, "Return")) return false;
                iterationPseudoCode = targetInfo.pseudoCode + '.return()';
                try { ir = (<{ (value?: GlideDuration): IteratorResult<string, GlideDuration>; }>targetInfo.iterator.return)(); }
                catch (e) {
                    atfHelper.setFailed('Unexpected exception while invoking ' + iterationPseudoCode, e);
            ...
            if (typeof onThrow === 'undefined') return true;
            
            if (iterations.length > 1) {
                if (!(createIterator.call(targetInfo, callInfo, onThrow, "Throw") && testIterations.call(targetInfo, iterations.length - 1))) return false;
                iterationPseudoCode = targetInfo.pseudoCode + '.throw("Error thrown!")';
                try { ir = (<{ (e?: any): IteratorResult<string, GlideDuration>; }>targetInfo.iterator.throw)("Error thrown!"); }
                catch (e) {
                    atfHelper.setFailed('Unexpected exception while invoking ' + iterationPseudoCode, e);
                    return false;
                }
            ...
            if (!createIterator.call(targetInfo, callInfo, onThrow, "Throw")) return false;
            iterationPseudoCode = targetInfo.pseudoCode + '.throw("Error thrown!")';
            try { ir = (<{ (e?: any): IteratorResult<string, GlideDuration>; }>targetInfo.iterator.throw)("Error thrown!"); }
            catch (e) {
                atfHelper.setFailed('Unexpected exception while invoking ' + iterationPseudoCode, e);
...
namespace site17Util_ReiterateTest {
    ...
    (function(outputs: sn_atf.ITestStepOutputs, steps: sn_atf.ITestStepsFunc, stepResult: sn_atf.ITestStepResult, assertEqual: sn_atf.IAssertEqualFunc): boolean {
        ...
        function createIterator(this: ITargetInfo, thisObj: IThisObj, onThrow?: GlideDuration | null, prefix?: string): boolean {
            thisObj.nextArgs = [];
            thisObj.reiterated = [];
            thisObj.throwCalled = false;
            thisObj.thrown = undefined;
            this.values = this.values.map(function(item: string | INextWithArg): string { return (typeof item === 'string') ? item : item.value });
            if (this.hasThisArg) {
                if (typeof onThrow !== 'undefined') {
                    if (onThrow === null) {
                        this.source = x_g_inte_site_17.Site17Util.iteratorFromArray(this.values, this.supportsReturn, this.finalReturnValue, function(e?: any): GlideDuration | undefined { 
                ...
                try {
                    this.iterator = x_g_inte_site_17.Site17Util.reiterate(this.source, function(this: IThisObj, value: string, next?: number, ...args: []) {
                ...
                if (typeof onThrow !== 'undefined') {
                    if (onThrow === null) {
                        this.source = x_g_inte_site_17.Site17Util.iteratorFromArray(this.values, this.supportsReturn, this.finalReturnValue, function(e?: any): GlideDuration | undefined { return; });
                        if (typeof this.finalReturnValue !== 'undefined')
                            this.pseudoCode = 'reiterate<string, GlideDuration, number>(iteratorFromArray(' + JSON.stringify(this.values) + ', ' + JSON.stringify(this.supportsReturn) + ', new GlideDuration("' +
                        ...
                        this.source = x_g_inte_site_17.Site17Util.iteratorFromArray(this.values, this.supportsReturn, this.finalReturnValue, function(e?: any): GlideDuration | undefined { return <GlideDuration>onThrow; });
                        if (typeof this.finalReturnValue !== 'undefined')
                            this.pseudoCode = 'reiterate<string, GlideDuration, number>(iteratorFromArray(' + JSON.stringify(this.values) + ', ' + JSON.stringify(this.supportsReturn) + ', new GlideDuration("' +
                ...
                try {
                    this.iterator = x_g_inte_site_17.Site17Util.reiterate(this.source, function(value: string, next?: number, ...args: []) {
        ...
        function testReiterate(iterations: (string | INextWithArg)[], supportsReturn: boolean, hasThisArg: boolean, finalReturnValue?: GlideDuration, onThrow?: GlideDuration | null): boolean {
            ...
            if (!(createIterator.call(targetInfo, thisObj, onThrow) && testIterations.call(targetInfo, iterations.length))) return false;

...
namespace site17Util_IteratorFromArrayTest {
    ...
    (function(outputs: sn_atf.ITestStepOutputs, steps: sn_atf.ITestStepsFunc, stepResult: sn_atf.ITestStepResult, assertEqual: sn_atf.IAssertEqualFunc): boolean {
        ...
        var idx: number;
        var ir: IteratorResult<string, number>;
        function onThrow(e?: any): number | undefined { return 12; }
        ...
        var iterator: Iterator<string, number>;
        try { iterator = x_g_inte_site_17.Site17Util.iteratorFromArray<string, number>(values, supportsReturn, finalReturnValue, onThrow); }
        catch (e) {
...
namespace site17Util_ReiterateTest {
    ...
    (function(outputs: sn_atf.ITestStepOutputs, steps: sn_atf.ITestStepsFunc, stepResult: sn_atf.ITestStepResult, assertEqual: sn_atf.IAssertEqualFunc): boolean {
        ...
        var values = iterations.map(function(d: INextWithArg): GlideDuration {
            return new GlideDuration(d.value);
        });
        var expectedNextArgs = iterations.map(function(d: INextWithArg): number | undefined {
            return d.arg;
        });
        var source: Iterator<GlideDuration, boolean, number> = x_g_inte_site_17.Site17Util.iteratorFromArray<GlideDuration, boolean, number>(values);
        function callbackFn(this: IThisObj, value: GlideDuration, arg?: number): void {
            this.nextArgs.push(arg);
        ...
        var idx: number;
        var arg: number | undefined;
        var ir: IteratorResult<GlideDuration, boolean>;
        var thisArg: IThisObj = { nextArgs: [] };
        ...
        var iterator: Iterator<GlideDuration, boolean, number>;
        try { iterator = x_g_inte_site_17.Site17Util.reiterate<GlideDuration, boolean, number>(source, callbackFn, thisArg); }
        catch (e) {
            atfHelper.setFailed('Unexpected exception while invoking ' + pseudoCode, e);
            return false;
        }
